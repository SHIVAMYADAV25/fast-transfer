  ...
  const startSend = async () => {
    ...
    try {
      ...
      const establishAndRun = async () => {
        ...
        // limitations" — this closes that gap).
        //
        // Connection count selection:
        //   - checkbox ticked  -> force the full MAX_PARALLEL_CONNECTIONS,
        //     for benchmarking/manual override.
        //   - checkbox unticked (default) -> chooseConnectionCount() picks
        //     automatically from the batch's total size, same thresholds
        //     ARCHITECTURE.md already documents (small files stay on 1
        //     connection since the extra ICE/DTLS/SCTP handshake per
        //     connection isn't worth it below ~8MB; large files get more).
        //     This was previously hardcoded to 1 regardless of size — the
        //     helper existed but nothing called it. That's the only change
        //     here: the receiver side (establishAndListen /
        //     establishParallelReceiverConnections below) already reads
        //     whatever connectionCount arrives in CONFIG generically, so it
        //     needed no changes to support this.
        const connectionCount = parallelMode
          ? MAX_PARALLEL_CONNECTIONS
          : chooseConnectionCount(resumeRef.current?.totalBytes ?? 0);
        signaling.send({
          type: "CONFIG",
          role: "sender",
          payload: { connectionCount },
